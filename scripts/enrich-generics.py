...
_TRAILING_CLAUSES = [
    re.compile(p, re.I) for p in [
        ...
        # "mit X% Fruchtanteil", "mit 5% Fett" — strip from "mit" onward
        r",?\s+mit\s+\d+[,.]?\d*\s*%.*",
        # Standalone percentage spec: "3,5% Fett", ",0% Zucker", etc.
        # Use \s (not \b) because "18% Bananen" has a space after %
        r",?\s*\d+[,.]?\d*\s*%\s.*",
        # Trailing size specs
        ...
        # Trailing flavor/ingredient suffix: "mit Erdbeere", "mit Himbeere-Holunderbeere",
        # "mit Stracciatellazubereitung, 3" etc. — noun (optionally hyphenated), plus
        # anything left over after a comma (orphaned digits, qualifiers like "mindestens").
        r"\s+mit\s+[A-ZÄÖÜ]\w*(?:[-][a-zäöüßA-ZÄÖÜ\w]*)*(,.*)?$",
    ]
]
...
def clean_generic(s: str) -> str:
    ...
    s = re.sub(r"\s*[,;]\s*$", "", s)
    s = re.sub(r"^\s*[,;]\s*", "", s)
    # Strip orphaned digits at end BEFORE preposition check
    # (e.g. "Joghurt mild mit 3" from ",5% Fett" partial match → "Joghurt mild mit")
    s = re.sub(r"[,\s]+\d+\s*$", "", s)
    # Now strip dangling prepositions/conjunctions exposed after digit removal
    s = re.sub(r"\s+(mit|aus|von|für|in|auf|zu|und|oder|sowie)\s*$", "", s, flags=re.I)
...
def process(in_path: Path, out_path: Path) -> None:
    ...
    for row in rows:
        ...
        # 1. Existing OFF generic_name (cleaned) → broad category
        if existing:
            result = clean_generic(existing)

        # 2. OFF broad tag mapping → broad category
        if not result:
    ...
    print(f"\n[generic_name_clean]  broad category")
